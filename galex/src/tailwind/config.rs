...
/// Generate a Tailwind v4 input CSS file with `@source` and `@theme` directives.
///
/// Tailwind v4 uses CSS-based configuration instead of `tailwind.config.js`.
/// Content detection uses `@source` directives to tell Tailwind where to
/// find class names.
///
/// Returns the path to the generated CSS input file.
pub fn generate_tailwind_config(
    ...
    let config_path = output_dir.join("input.css");

    let mut css = String::new();
    css.push_str("/* Generated by GaleX — Tailwind v4 configuration */\n");
    ...
    // Theme customisation via @theme block
    let mut theme_vars = Vec::new();
    if let Some(ref primary) = config.primary {
        theme_vars.push(format!("  --color-primary: {};", primary));
    }
    if let Some(ref font) = config.font_sans {
        theme_vars.push(format!("  --font-sans: {};", font));
    }
    ...
    // Safelist — reference classes in comments so Tailwind detects them
    if !safelist.is_empty() {
        css.push_str("\n/* Safelist: classes that must always be generated */\n");
    ...
    std::fs::write(&config_path, &css).ok();
    config_path
}

/// Generate the default Tailwind v4 input CSS.
///
/// Tailwind v4 uses `@import "tailwindcss"` instead of the v3
...
"##;
    ...
    fn generate_config_file() {
        ...
        // v4: uses CSS-based config with @import, @source, @theme
        assert!(
            content.contains("@import \"tailwindcss\""),
            "has v4 import: {content}"
        );
        assert!(
            content.contains("@source"),
            "has source directive: {content}"
        );
        ...
        assert!(
            content.contains("--color-primary"),
            "has theme var: {content}"
        );
        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn default_css_has_v4_import() {
