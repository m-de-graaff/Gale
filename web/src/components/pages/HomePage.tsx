...
function TypeGraphSvg() {
  return (
    <svg viewBox="0 0 200 120" fill="none" className="w-full h-full opacity-70">
      <circle cx="40" cy="30" r="14" className="fill-accent/10 stroke-accent/30" strokeWidth="1" />
      <circle cx="100" cy="20" r="14" className="fill-accent/10 stroke-accent/30" strokeWidth="1" />
      <circle cx="160" cy="35" r="14" className="fill-accent/10 stroke-accent/30" strokeWidth="1" />
      <circle cx="60" cy="80" r="14" className="fill-accent/10 stroke-accent/30" strokeWidth="1" />
      <circle cx="130" cy="90" r="14" className="fill-accent/10 stroke-accent/30" strokeWidth="1" />
      <line x1="52" y1="37" x2="88" y2="23" className="stroke-accent/15" strokeWidth="1" />
      <line x1="112" y1="27" x2="148" y2="32" className="stroke-accent/15" strokeWidth="1" />
      <line x1="46" y1="43" x2="55" y2="68" className="stroke-accent/15" strokeWidth="1" />
      <line x1="106" y1="33" x2="126" y2="77" className="stroke-accent/15" strokeWidth="1" />
      <line x1="72" y1="83" x2="118" y2="88" className="stroke-accent/15" strokeWidth="1" />
      <text x="40" y="34" textAnchor="middle" className="fill-accent/80 text-[7px] font-mono">int</text>
      <text x="100" y="24" textAnchor="middle" className="fill-accent/80 text-[7px] font-mono">Signal</text>
      <text x="160" y="39" textAnchor="middle" className="fill-accent/80 text-[7px] font-mono">Guard</text>
      <text x="60" y="84" textAnchor="middle" className="fill-accent/80 text-[7px] font-mono">string</text>
      <text x="130" y="94" textAnchor="middle" className="fill-accent/80 text-[7px] font-mono">fn()</text>
    </svg>
  )
}
...
function BinaryOutputSvg() {
  return (
    <svg viewBox="0 0 240 100" fill="none" className="w-full h-full opacity-70">
      <rect x="10" y="15" width="32" height="40" rx="4" className="fill-accent/5 stroke-accent/20" strokeWidth="1" />
      <rect x="50" y="15" width="32" height="40" rx="4" className="fill-accent/5 stroke-accent/20" strokeWidth="1" />
      <rect x="90" y="15" width="32" height="40" rx="4" className="fill-accent/5 stroke-accent/20" strokeWidth="1" />
      <text x="26" y="39" textAnchor="middle" className="fill-accent/50 text-[6px] font-mono">.gx</text>
      <text x="66" y="39" textAnchor="middle" className="fill-accent/50 text-[6px] font-mono">.gx</text>
      <text x="106" y="39" textAnchor="middle" className="fill-accent/50 text-[6px] font-mono">.gx</text>
      <path d="M130 35 L155 35" className="stroke-accent/25" strokeWidth="1.5" strokeLinecap="round" markerEnd="url(#ah)" />
      <defs><marker id="ah" markerWidth="6" markerHeight="4" refX="5" refY="2" orient="auto"><polygon points="0 0, 6 2, 0 4" className="fill-accent/25" /></marker></defs>
      <rect x="162" y="10" width="65" height="50" rx="6" className="fill-accent/8 stroke-accent/30" strokeWidth="1.5" />
      <text x="194" y="30" textAnchor="middle" className="fill-accent text-[7px] font-mono font-bold">gale_app</text>
      <text x="194" y="42" textAnchor="middle" className="fill-accent/35 text-[5px] font-mono">4.2 MB</text>
      <text x="66" y="72" textAnchor="middle" className="fill-[rgba(255,255,255,0.25)] text-[6px]">source files</text>
      <text x="194" y="76" textAnchor="middle" className="fill-[rgba(255,255,255,0.25)] text-[6px]">single binary</text>
    </svg>
  )
}
...
function TerminalCommand() {
  ...
  return (
    <div
      onClick={handleCopy}
      className="inline-flex items-center gap-3 px-5 py-2.5 rounded-lg border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.02)] cursor-pointer hover:bg-[rgba(255,255,255,0.04)] transition-colors select-none"
    >
      <span className="text-muted-foreground font-mono text-[13px]">$</span>
      <span className="text-foreground font-mono text-[13px]">{cmd}</span>
      {copied ? <Check className="w-3.5 h-3.5 text-accent" /> : <Copy className="w-3.5 h-3.5 text-muted-foreground/40" />}
    </div>
  )
}
  ...
  return (
    <div className="flex-1">
      ...
      <section className="relative overflow-hidden">
        <div className="absolute top-[-50%] left-1/2 -translate-x-1/2 w-[1000px] h-[1000px] rounded-full opacity-[0.07]" style={{
          background: 'radial-gradient(circle, hsl(162 72% 46%) 0%, transparent 60%)',
        }} />
            ...
            >
              {TITLE_WORDS.map((word, i) => (
                <motion.span
                  ...
                  className={`inline-block mx-1 sm:mx-2 ${word === '.GX' ? 'bg-gradient-to-r from-accent to-emerald-300 bg-clip-text text-transparent' : ''}`}
                >
                  {word}
                </motion.span>
              ))}
      ...
      {/* ── Built on ─────────────────────────────────────────────────── */}
      <section className="border-y border-[rgba(255,255,255,0.06)]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
          <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-3">
            <span className="text-[11px] uppercase tracking-widest text-muted-foreground/40 font-medium">Built on</span>
            {BUILT_ON.map(t => (
              <div key={t.name} className="flex items-center gap-2">
                <span className="text-[13px] font-medium text-foreground/80">{t.name}</span>
                <span className="text-[11px] text-muted-foreground/40 hidden sm:inline">{t.desc}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {METRICS.map(m => (
              <div key={m.label}>
                ...
                <div className="text-[11px] text-muted-foreground/40 mt-0.5">{m.detail}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-6 gap-3">
          <Card className="col-span-full lg:col-span-4">
            <CardContent className="grid sm:grid-cols-2 gap-5 pt-6">
              <div className="flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <div className="flex items-center justify-center w-8 h-8 rounded-full border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.03)]">
              ...
              <div className="rounded-lg overflow-hidden border border-[rgba(255,255,255,0.06)]"><CodeBlock code={BOUNDARY_CODE} language="gx" /></div>
            </CardContent>
          </Card>
          ...
          <Card className="col-span-full sm:col-span-3 lg:col-span-3">
            <CardContent className="pt-6">
              ...
              <p className="text-[13px] text-muted-foreground leading-relaxed mb-3">Typed schemas generating both Rust and JS validation. Compose via <code className="text-accent/80 bg-[rgba(255,255,255,0.05)] px-1 rounded text-[12px]">.partial()</code>, <code className="text-accent/80 bg-[rgba(255,255,255,0.05)] px-1 rounded text-[12px]">.pick()</code>, <code className="text-accent/80 bg-[rgba(255,255,255,0.05)] px-1 rounded text-[12px]">.omit()</code>.</p>
              <div className="rounded-lg overflow-hidden border border-[rgba(255,255,255,0.06)] text-[12px]"><CodeBlock code={GUARD_CODE} language="gx" /></div>
            </CardContent>
          </Card>
          ...
          <Card className="col-span-full sm:col-span-3 lg:col-span-3">
            <CardContent className="pt-6">
              ...
              <p className="text-[13px] text-muted-foreground leading-relaxed"><code className="text-accent/80 bg-[rgba(255,255,255,0.05)] px-1 rounded text-[12px]">gale build</code> compiles .gx into a standalone Rust binary. No Node.js, no runtime deps.</p>
            </CardContent>
          </Card>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-24">
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <div>
              ...
              <p className="text-[14px] text-muted-foreground leading-relaxed mb-5">The SDK installer puts <code className="text-accent/80 bg-[rgba(255,255,255,0.05)] px-1 py-0.5 rounded text-[12px]">gale</code> and <code className="text-accent/80 bg-[rgba(255,255,255,0.05)] px-1 py-0.5 rounded text-[12px]">gale-lsp</code> on your PATH.</p>
            ...
            <Link key={card.href} to={card.href}>
              <Card className="h-full cursor-pointer group">
                <CardContent className="pt-5 pb-5">
                  ...
                  <p className="text-[12px] text-muted-foreground/60 leading-relaxed">{card.desc}</p>
                  <span className="inline-flex items-center gap-1 mt-3 text-[12px] text-accent/60 group-hover:text-accent transition-colors">Read docs <ArrowRight className="w-3 h-3" /></span>
                </CardContent>
              </Card>
            </Link>
          ))}
